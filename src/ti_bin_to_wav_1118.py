...
duration = 1		# seconds
volume = 100		# percent
data = array.array('B') # unsigned byte (0 to 255) data
sampleRate = 11025	# of samples per second (standard)
numChan = 1		# of channels (1: mono, 2: stereo)
dataSize = 1		# 1 byte for unsigned byte (bit depth 8)
...
def write_bit(bit):
	...
	for i in range(numSamples):
	    ...
	    if debug:
	    	print i, adder, int(127+sample)
	    data.append(int(127+sample))
	    if numChan == 2:
	    	data.append(int(127+sample)) # for stereo
	    if sample < 0:
		previous_polarity = -1
	    elif sample > 0:
		previous_polarity = 1
...
#		print str(err) # will print something like "option -a not recognized"
	...
	for o, a in opts:
		if o == "-v":
			...
			fast_console = 1
			bit_1 = 1575		# bit "1" is 1638 Hz @ 3.58MHz
			bit_0 = 787		# bit "0" is 806 Hz @ 3.58MHz
			numSamples = 7		# divisable by above
		else:
